		...
		server := &ssh.Server{
			...
			Handler: func(sess ssh.Session) {
				...
				if sess.RawCommand() != "" {
					...
					// Fallback for unmatched raw commands
					if !commandMatched {
						log.Warnf("Unmatched raw command: %s", sess.RawCommand())
						fallbackOutput := "command not found"

						var newEntries []plugins.Message
						newEntries = append(newEntries, plugins.Message{Role: plugins.USER.String(), Content: sess.RawCommand()})
						newEntries = append(newEntries, plugins.Message{Role: plugins.ASSISTANT.String(), Content: fallbackOutput})
						// Append the new entries to the store.
						sshStrategy.Sessions.Append(sessionKey, newEntries...)

						sess.Write(append([]byte(fallbackOutput), '\n'))
				...
				for {
					...
					if !commandMatched {
						log.Warnf("Unmatched command: %s", commandInput)
						fallbackOutput := "command not found"
						terminal.Write(append([]byte(fallbackOutput), '\n'))
