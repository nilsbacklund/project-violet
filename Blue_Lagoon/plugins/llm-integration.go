...
const (
	...
	systemPromptVirtualizeTCPService    = "You will act as a generic TCP service. The user will send raw TCP payloads, and you are to reply with what the service should show. Your responses must be plain text. Do not provide explanations or type commands unless explicitly instructed by the user."
...
func (llmHoneypot *LLMHoneypot) buildPrompt(command string) ([]Message, error) {
	...
	case tracer.TCP:
		prompt = systemPromptVirtualizeTCPService
		if llmHoneypot.CustomPrompt != "" {
			prompt = llmHoneypot.CustomPrompt
		}
		messages = append(messages, Message{
		...
		// For TCP, we don't have a predefined initial user/assistant exchange like SSH or HTTP.
		// We'll just use the system prompt and the user's command.
		// If history is added for TCP later, this is where it would be appended.
	default:
